...
class TestCLINewCommands:
    ...
    def test_removed_research_command_is_not_public(self):
        runner = CliRunner()
        removed_command = "h" + "unt"
        result = runner.invoke(main, [removed_command])
        help_result = runner.invoke(main, ["--help"])

        assert result.exit_code != 0
        assert f"Path '{removed_command}' does not exist" in result.output
