...
_SINK_RULES = (
    ...
    _SinkRule("subprocess.check_call", "command_execution", "subprocess execution", 40, suffix=True),
    _SinkRule("subprocess.check_output", "command_execution", "subprocess execution", 40, suffix=True),
...
def _score_function(
    *,
    ...
    annotated = [p.name for p in params if p.annotation in _TEXT_TYPES | _BYTES_TYPES | _STRUCTURED_TYPES]
    if annotated:
        signals.append(
            HuntSignal(
...
def _recommend_harness(
    *,
    ...
    return "atheris", "single fuzzable input can be exercised locally", target_param.name, input_kind
...
def _extract_fuzz_introspector_hits(data: dict[str, object]) -> list[dict[str, object]]:
    ...
    for item in raw_hits:
        if not isinstance(item, dict):
            continue
        name = _optional_str(
            item.get("func_name") or item.get("function_name") or item.get("name")
        )
        if not name:
            continue
        ...
        reach_state = "reached" if reached_by and call_loc.lower() != "not in call tree" else "not_reached"
        hits.append(
            {
                "name": name,
